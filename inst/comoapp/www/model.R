...
# START Define the parameters and link to inputs ----
parameters <- c(
  p = input$p,
  rho = input$rho,
  omega = input$omega,
  ...
  ihr_scaling = input$ihr_scaling,  # addition v12
  nus = input$nus,
  nusc = input$nusc,
  nu_icu = input$nu_icu,
  ...
  screen_overdispersion = input$screen_overdispersion,  # addition v12
  screen_contacts = input$screen_contacts,  # addition v12
...
# END Define the parameters and link to inputs ----
  ...
  with(as.list(c(Y, parameters)),
       {
         ...
         # critV <- 0
         # 
         # critH <- 1-1/(1+(sum(H)/KH)^20)
         # crit <- 1-1/(1+(sum(ICU)/KICU)^20)
         # critV <- 1-1/(1+(sum(Vent)/Kvent)^20)
         ...
         dVentCdt <- gamma*ihr[,2]*prob_icu*critV*(1-crit)*prob_vent*E + gamma*ihr[,2]*prob_icu*critV*(1-crit)*prob_vent*QE - nu_ventc*VentC +ageing%*%VentC - mort*VentC 
...
process_ode_outcome <- function(out){
  ...
  print(paste0("round(100 * tail((rowSums(out[,(Rindex+1)])),1)/sum(popstruc[,2]), 1)", 
               " -->",  
               round(100 * tail((rowSums(out[,(Rindex+1)])),1)/sum(popstruc[,2]), 1)))
